...
class GenerateFrameIndiceswithPadding(object):
    ...
    Args:
        ...
        filename_tmpl (str): Template for file name. Default: '{:03d}'.
    """

    def __init__(self, padding, filename_tmpl='{:03d}'):
    ...
    def __call__(self, results):
        ...
        # changed
        for i in range(current_idx - num_pad, current_idx + num_pad + 1):
            if osp.exists(osp.join(gt_path_root, clip_name, f'{i:03d}.png')):
                pad_idx = i
            else:
                pad_idx = current_idx
            frame_list.append(pad_idx)
...
class GenerateFrameIndices(object):
    ...
    def __call__(self, results):
        ...
        # judge frame exist
        while not osp.exists(osp.join(gt_path_root, clip_name, f'{start_frame_idx:03d}.png')) or \
                not osp.exists(osp.join(gt_path_root, clip_name, f'{end_frame_idx:03d}.png')):
            # while (start_frame_idx < 1) or (end_frame_idx > self.frames_per_clip):
            center_frame_idx = np.random.randint(1, self.frames_per_clip + 1)
            start_frame_idx = center_frame_idx - num_half_frames * interval
            end_frame_idx = center_frame_idx + num_half_frames * interval
        ...
        lq_path = [
            osp.join(lq_path_root, clip_name, f'{v:03d}.png')
