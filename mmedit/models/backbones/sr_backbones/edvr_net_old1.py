...
class PredeblurModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = ConvModule(
            num_in_ch, num_feat, 3, 1, 1, act_cfg=act_cfg)
        if self.hr_in:
            # downsample x4 by stride conv
            self.stride_conv_hr1 = ConvModule(
                num_feat, num_feat, 3, 2, 1, act_cfg=act_cfg)
        ...
        self.resblock_l3 = ResidualBlockNoBN(mid_channels=num_feat)
        self.resblock_l2_1 = ResidualBlockNoBN(mid_channels=num_feat)
        self.resblock_l2_2 = ResidualBlockNoBN(mid_channels=num_feat)
        self.resblock_l1 = nn.ModuleList(
            [ResidualBlockNoBN(mid_channels=num_feat) for i in range(5)])
    ...
    def __init__(self,
        ...
        if self.with_predeblur:
            self.predeblur = PredeblurModule(
                num_feat=mid_channels, hr_in=self.hr_in)
