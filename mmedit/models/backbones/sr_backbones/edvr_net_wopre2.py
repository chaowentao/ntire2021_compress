...
class EDVRNet_WoPre2(nn.Module):
    ...
    def __init__(self,
                 ...
                 with_nonlocal=False,
                 with_car=False):
        super(EDVRNet_WoPre2, self).__init__()
        self.center_frame_idx = center_frame_idx
        self.hr_in = hr_in
        ...
        if self.with_car:
            ...
            self.feature_extraction = make_layer(
                ...
                num_blocks_extraction,
                mid_channels=mid_channels)
        self.down_sample = nn.Conv2d(mid_channels, mid_channels, 3, 2, 1)
        ...
        self.channel_convert = nn.Conv2d(2 * mid_channels, mid_channels, 1, 1)
        self.channel_convert2 = nn.Conv2d(mid_channels, 64, 1, 1)
    ...
    def forward(self, x):
        ...
        # top_l_in = x[:, :, :, :256, :480]
        # top_r_in = x[:, :, :, :256, (960-480):960]
        # down_l_in = x[:, :, :, 244:500, :480]
        # down_r_in = x[:, :, :, 244:500, 480:960]
        #
        # x1 = torch.cat((top_l_in, top_r_in), 2)
        # x2 = torch.cat((down_l_in, down_r_in), 2)
        # x = torch.cat((x1, x2), 2)
        n, t, c, h, w = x.size()
        ...
        # L1
        if self.with_predeblur:
            l1_feat = self.conv_1x1(self.predeblur(x.view(-1, c, h, w)))
            if self.hr_in:
                h, w = h // 2, w // 2
        else:
            l1_feat = self.lrelu(self.conv_first(x.view(-1, c, h, w)))

        l1_feat_ = self.feature_extraction(l1_feat)
        l1_feat = self.down_sample(l1_feat_)
        # L2
        ...
        if self.with_nonlocal:
            aligned_feat = self.non_local(aligned_feat)
        ...
        l = l1_feat_.view(n, t, -1, 2 * h,
                          2 * w)[:, self.center_frame_idx, :, :, :].clone()
        feat = torch.cat((l, out), 1)
        feat = self.channel_convert(feat)
        out = self.reconstruction(feat)
        # out = self.lrelu(self.upsample1(out))
        # out = self.lrelu(self.upsample2(out))
        out = self.channel_convert2(out)
        out = self.lrelu(out)
        # out = self.lrelu(out)
        ...
        # out_ = base.mul(0.0)
        # out_[:, :, :256, :480] = top_l_out
        # out_[:, :, :256, (960 - 480):960] = top_r_out
        # out_[:, :, 244:500, :480] = down_l_out
        # out_[:, :, 244:500, 480:960] = down_r_out
        out += base
        return out
